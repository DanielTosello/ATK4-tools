...
==ATK4===================================================
   ...
   This version of the class has been altered to support
   MSSQL/dblib/FreeTDS by removing init parameters from the
   connection string, as they are unsupported by dblib. 
 =====================================================ATK4=*/
class DB extends AbstractController {
	 ...
	 * by default.
	 * If you want to specify a different one, add this parameter
	...
	/**
	 * Connect will parse supplied DSN and connect to the database.
	 * Please be
	 ...
	 *        	Connection string in PEAR::DB or PDO format
	 *        	
	 * @return DB $this
	 */
	function connect($dsn = null) {
		...
		if (is_string ( $dsn )) {
			...
			if (is_string ( $dsn )) {
				...
				$dsn = array (
						$matches [1] . ':host=' . $matches [5] . ';dbname=' . $matches [7] . ';charset=utf8',
						$matches [2],
						$matches [4] /*,
                    
                    // Altered for compatability with MSSQL/dblib/FreeTDS.
                    // The following line will cause a warning (setting attributes
                    // not supported) which will affect ATK4 and prevent it
                    // executing properly
		...
		try {
			$this->dbh = new PDO ( @$dsn [0], @$dsn [1], @$dsn [2] )			// ,
			         //      @$dsn[3] ?: array(PDO::ATTR_PERSISTENT => true)
                    	)// @$dsn[3] ?: array(PDO::ATTR_PERSISTENT => true)
			null;
		} catch ( PDOException $e ) {
			throw $this->exception ( 'Database Connection Failed' )->addMoreInfo ( 'PDO error', $e->getMessage () )->addMoreInfo ( 'DSN', $dsn [0] );
			;
		}
		
		// Configure PDO to play nice
                    $this// Configure PDO to play nice
		null->dbh->setAttribute ( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
		
		// Based on type, switch dsql class
                    list// Based on type, switch dsql class
		null ( $this->type, $junk ) = explode ( ':', $dsn [0] );
		$this->type = strtolower ( $this->type );
		$this->dsql_class = isset ( $dsn ['class'] ) ? $dsn ['class'] : 'DB_dsql_' . $this->type;
		//temp mssql fix
                    $this// temp mssql fix
		null->dbh->exec ( 'SET QUOTED_IDENTIFIER ON; SET ANSI_WARNINGS ON' );
		return $this;
	}
	// }}}
                     ...
                     * */
                    // }}}
	nullfunction dsql($class = null) {
		$class = $class ?  : $this->dsql_class;
		$obj = $this->add ( $class );
		if (! $obj instanceof DB_dsql) {
			throw $this->exception ( 'Specified class must be descendant of DB_dsql' )->addMoreInfo ( 'class', $class );
		}
		return $obj;
	}
	// }}}
                    	 ...
                    	 */
                    	// }}}
	nullfunction query($query, $params = array()) {
		// If user forgot to explicitly connect to database, let's do it for him
                    		if// If user forgot to explicitly connect to database, let's do it for him
		null (! $this->dbh) {
			$this->connect ();
		}
		...
		// sure we operate with strings here
                    		if// sure we operate with strings here
		null (is_object ( $query )) {
			$query = ( string ) $query;
		}
		...
		// the error, but only if error=17.
                    		$e// the error, but only if error=17.
		null = null;
		for($i = 0; $i < 2; $i ++) {
			try {
				...
				foreach ( $params as $key => $val ) {
					if (! $statement->bindValue ( $key, $val, is_int ( $val ) ? (PDO::PARAM_INT) : (PDO::PARAM_STR) )) {
						throw $this->exception ( 'Could not bind parameter' )->addMoreInfo ( 'key', $key )->addMoreInfo ( 'val', $val )->addMoreInfo ( 'query', $query );
					}
				}
	 ...
	 * Driver-dependant. Redefine if needed,
	 * but avoid using.
	 *
	...
	public function beginTransaction() {
        ...
        if// transaction starts only if it was not started before
		null ($this->transaction_depth == 1) {
	...
	public function commit() {
    	...
    	if// This means we rolled something back and now we lost track of commits
		null ($this->transaction_depth < 0) {
